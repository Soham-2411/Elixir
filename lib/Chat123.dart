                    ...
                    FlatButton(
                      onPressed: () async{
                        if (messageText.length < 5) {
                          ...
                          TranslationResult translationResult = await service.translate(messageText, Language.ENGLISH, Language.HINDI);
                          messageTextController.clear();
                          _firestore.collection('messages').add({
                            'text': messageText,
                            'htext': translationResult.toString(),
                            'sender': email,
                            'time': DateTime.now(),
                          });
                          messageText = '';
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        for (var message in messages) {
          ...
          final hmessageText = message.data()['htext'];
...
class MessageBubble extends StatefulWidget {
  MessageBubble({this.sender, this.text, this.htext, this.isMe});
